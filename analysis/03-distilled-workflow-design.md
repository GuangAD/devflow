...
- **seam 纪律**:测试只写在门 1 确认过的 seam 上;seam 接口在 spec.md 有规格条目(四件套:签名/后置条件/副作用与可重入性/错误行为)+ pinned case;期望值来自独立事实源(手算/外部锚点/构造反例),禁止同义反复断言。接口面穷举清单覆盖全部公共导出;实现期计划外导出与规格修订一律登记 spec.md 变更记录;spec.md 与 design.md 冲突以 design.md 为准,触及 design 决策走回退协议。机检铁律的判定域是数值等价与结构自洽,书写惯例(零合并/前缀/可省略成分)不在其内,须由矩阵代表行的字面期望值钉住——惯例错不改数值,往返枚举再大也测不出。
...
- 一切执行状态在 plan.md(勾选 + commit 哈希 + 开发记录节——块格式:ledger 首行 + 字段缩进子行,空字段省略)与 spec.md 变更记录(接口真值 ledger,块格式:一条一块,过长在门 3 折叠),不在会话上下文里。
