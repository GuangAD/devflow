...
一个 `/dev` 命令走天下的个人 AI 编码工作流:内部分类器路由轻/重流程,三道门(设计→计划→最终)守住方向、路径、结果,门之间 agent 挂机自治。内置 TDD 红绿循环、子代理独立评审、证据先于声称等纪律技能。
...
安装后重启 pi,输入 `/` 应能看到 `dev`、`dev-doc`、`dev-resume`、`dev-review` 四个命令。
