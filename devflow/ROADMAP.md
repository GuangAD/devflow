...
- 2026-09-15 新增 `/dev-map` 决策地图(补采上游 wayfinder,原列舍弃清单 ①「场景不符」):
  - 新增 `skills/dev-map/SKILL.md` + `skills/dev-map/references/map-template.md` + `prompts/dev-map.md`;地图为 `docs/maps/<日期>-<slug>/map.md` 单文件,五节(目的地/备注/已定决策/雾/越界),票用 checkbox + commit 哈希,认领降级为「每会话一票+解完即提交」
  - `dev/SKILL.md`:分类器二分→三分(新增判巨信号:一次访谈装不下、有说不 sharp 的区域;判巨优先于判重),新增「巨流程(路由到 /dev-map)」节,阶段 1 增加「已有地图时已定决策直接作为访谈既有共识」,升级棘轮改为只可轻→重→巨单向升,frontmatter description 同步
  - `analysis/03`:命令集与分类器改三分;新增第六节「巨流程(地图层)」,原六~十一节顺延为七~十二(同步修正文内「见第六节」与本文件此处的节号引用);第十一期不做移除 wayfinder;第十节知识沉淀新增 docs/maps;来源表新增 wayfinder 行(标注原列舍弃、2026-09-15 补采)
