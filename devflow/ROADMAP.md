...
- 2026-09-21 03:19 覆盖声明两项缺口修复(源自 pi-web-new 对照旧项目基线的实证复盘):
...
- 2026-09-23 pi-converter 实证回流(可读性改造首验,门 3 完成):
  - `spec-template.md`「变更记录」节改**块格式**(一条一块,附示例;过长按门 3 折叠纪律处理)——改造时漏掉的最后一个管道焊接点清零
  - `spec-template.md` pinned case 注释**咬死 mutation check 范围**:仅缺陷补写类须反向验证,常规用例只走默认制「亲眼看红」——pi-converter 生成的 spec 曾把「须留 mutation check」放宽到全体 pinned case,属过度执行
  - `analysis/03` 第九节同步(变更记录块格式注记)

- 2026-09-23 覆盖盲区教训回填(源自「段形态×组位 8 基线 7/8 无字面钉住」分析:往返/守恒类铁律判数值等价与结构自洽,书写惯例不在其判定域,惯例漏零不改数值、4 万条枚举往返全绿也测不出):
  - `spec-template.md` 机检铁律 bullet 末尾加「判定域提醒」:惯例类规则(零合并、固定前缀、可省略成分)必须靠矩阵代表行字面期望值钉住,不能只交铁律兜底
  - `design-checklist.md` 新增自查条目:书写惯例类规则有字面锚(附 8 基线教训)
  - `analysis/03` 第七节 seam 纪律同步同句
  - 注:机检铁律行因全半角标点差异经「ASCII 锚点 + 载荷文件」方式写入(Edit/PowerShell 字面量匹配均失败,字节级核实文件内容正常)
